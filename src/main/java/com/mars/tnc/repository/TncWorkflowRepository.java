...
import com.mars.tnc.model.TncWorkflow;
import feign.Param;
...
@Repository
public interface TncWorkflowRepository extends JpaRepository<TncWorkflow, Long> {

    @Query(value = "SELECT tw FROM TncWorkflow tw " +
            "WHERE tw.tncTypeId = :tncTypeId")
    List<TncWorkflow> findByTncTypeId(@Param("tncTypeId") Long tncTypeId);
}
