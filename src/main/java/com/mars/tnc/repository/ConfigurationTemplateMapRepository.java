...
import com.mars.tnc.model.ConfigurationTemplateMap;
...
@Repository
public interface ConfigurationTemplateMapRepository extends JpaRepository<ConfigurationTemplateMap, Long> {

    @Query(value = "SELECT COUNT(ctm) > 0 FROM ConfigurationTemplateMap ctm " +
            "WHERE ctm.referenceTypeId = :referenceTypeId AND " +
            "ctm.referenceId = :referenceId")
    Boolean existsByReferenceTypeIdAndReferenceId(@Param("referenceTypeId") Long referenceTypeId,
