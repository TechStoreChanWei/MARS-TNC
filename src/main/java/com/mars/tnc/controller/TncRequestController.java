...
import com.mars.tnc.config.security.TokenProvider;
...
@RequestMapping(ApiConstants.TNC_REQUEST_BASE_URL)
public class TncRequestController {

    private final TncRequestService tncRequestService;
    private final TokenProvider tokenProvider;

    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody TncRequestCreateDTO tncRequestCreateDTO) {
        //String token = this.tokenProvider.resolveToken(request);
        //Long userId = this.tokenProvider.getUserIdFromToken(token);
        Long userId = 1L;
        TncRequestDTO tncRequestDTO = tncRequestService.create(tncRequestCreateDTO, userId);
        return new ResponseEntity<>(tncRequestDTO, HttpStatus.OK);
    }
}
